...
const FeaturedSection = () => {
  ...
  const fetchData = async () => {
    try {
      const blogRes = await axios.get("http://localhost:3000/api/v1/blogs/featured");
    ...
    try {
      const newsRes = await axios.get("http://localhost:3000/api/v1/news/articles");
    ...
    try {
      const eventRes = await axios.get("http://localhost:3000/api/v1/events");
        ...
        {featuredBlog && (
          <div className="cosmic-card overflow-hidden shadow-lg cursor-pointer">
            ...
            <p className="uppercase text-xs font-bold tracking-widest text-space-accent mb-2">
              ...
              <h3 className="text-lg sm:text-xl font-bold mb-2">{featuredBlog.title}</h3>
              ...
              <p className="text-xs text-gray-500">Author:  {(featuredBlog.author?.name).toUpperCase()}</p>
            </div>
            </Link>
          </div>
        )}
        ...
        {featuredNews && (
          <div className="cosmic-card overflow-hidden shadow-lg cursor-pointer">
            ...
            <p className="uppercase text-xs font-bold tracking-widest text-space-accent mb-2">
              ...
              <h3 className="text-lg sm:text-xl font-bold mb-2">{featuredNews.title}</h3>
              <p className="text-sm text-gray-400 mb-3 sm:block">
                {featuredNews.summary.slice(0, 150)}...
              </p>
              <p className="text-xs text-gray-500">Source: {featuredNews.news_site}</p>
            </div>
            </Link>
          </div>
        )}
        ...
        {upcomingEvent && (
          <div className="cosmic-card overflow-hidden shadow-lg cursor-pointer">
            ...
            <p className="uppercase text-xs font-bold tracking-widest text-space-accent mb-2">
              ...
              <h3 className="text-lg sm:text-xl font-bold mb-2">{upcomingEvent.title}</h3>
              <p className="text-sm text-gray-400 mb-2">
              ...
              <p className="text-xs text-gray-500">Venue: {upcomingEvent.location}</p>
            </div>
            </Link>
          </div>
        )}
